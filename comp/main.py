import sys
sys.path.append('/data/colin/DRNet')
import os
import random
...
from comp_set import CompDataModule
from comp_net import CompNet
...
def main(cfg):
    ...
    if cfg.task.mode == 'train':
        if cfg.task.resume:
            model = CompNet(cfg)  # DRNet.load_from_checkpoint(cfg.weight)
        else:
            if not cfg.task.finetune or not os.path.exists(cfg.weight):
                model = CompNet(cfg)  # train from scratch
            else:   # finetuning based on previous weights
                # set strict to False to ignore the missing params in the previous phase
                model = CompNet.load_from_checkpoint(cfg.weight, config=cfg, strict=False)
    elif cfg.task.mode == 'test':
        if os.path.exists(cfg.weight):
            model = CompNet.load_from_checkpoint(cfg.weight, config=cfg)
        else:
            model = CompNet(cfg)  # test from scratch
    else:
        raise ValueError(f'Invalid task mode: {cfg.task.mode}')
    
    datamodule = CompDataModule(cfg)
