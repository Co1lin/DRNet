...
class DetectionLoss(BaseLoss):
    def __call__(self, est_data, gt_data, dataset_config):
        ...
        return {'total': loss,
                ...
                'sem_cls_loss': sem_cls_loss.detach(),
                'pos_ratio': pos_ratio.detach(),
                'neg_ratio': neg_ratio.detach(),
                'center_loss': center_loss.detach(),
                'heading_cls_loss': heading_cls_loss.detach(),
                'heading_reg_loss': heading_reg_loss.detach(),
                'size_cls_loss': size_cls_loss.detach(),
                'size_reg_loss': size_reg_loss.detach(),
                'obj_acc': obj_acc.detach()}
...
class PCN_Loss(BaseLoss):
    ...
    def __call__(self, pred_fine, pred_coarses, full_scan, full_scan_coarse):
        ...
        return self.weight * errG, CD_LOSS.detach()
...
def compute_objectness_loss_boxnet(est_data, gt_data):
    ...
    objectness_mask = torch.ones((objectness_label.shape[0], objectness_label.shape[1])) # no ignore zone anymore
