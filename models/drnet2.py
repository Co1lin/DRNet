...
class DRNet(pl.LightningModule):
    ...
    def __init__(self, config = None):
        ...
        if self.cfg.phase == 'completion':
            ...
            self.completions = nn.ModuleList([
    ...
    def forward(self, batch):
        with torch.no_grad():
            ...
            if self.cfg.phase == 'completion':
                ...
                num_objs = batch_size * N_proposals
                cls_codes_for_completion = cls_codes_for_completion.view(num_objs, -1)
                
                completion_loss = 0
                shape_example = None
                
                # p = Pool(processes=num_processes)
                # compl_losses = p.map(partial(compute_compl_loss, 
                #     cls_codes_for_completion, object_input_features,
    ...
    def _common_step(self, batch, batch_idx):
        ...
        if self.cfg.phase == 'completion':
            ...
            object_input_features = object_input_features.transpose(1, 2) \
                                    .contiguous().view(num_objs, feat_dim)
            completion_loss = 0
            shape_example = None
    ...
    def configure_optimizers(self):
        ...
        if self.cfg.phase == 'completion':
            optimizers['completion'] = optim.AdamW(
                ...
                list(self.completions.parameters()),
    ...
    def _compute_loss(self, est_data, gt_data):
        ...
        if self.cfg.phase == 'completion':
            ...
            total_loss = {**detection_loss,
                        'detection_loss': detection_loss['total'].clone(),
...
# def compute_compl_loss(completions, cls_codes_for_completion, object_input_features,
#                                 input_points_for_completion, input_points_occ_for_completion, index):
#     net_idx = torch.argmax(cls_codes_for_completion[index])
#     compl_loss, _ = completions[net_idx].compute_loss(
#         torch.unsqueeze(object_input_features[index], 0),
#         torch.unsqueeze(input_points_for_completion[index], 0),
#         torch.unsqueeze(input_points_occ_for_completion[index], 0),
#         torch.unsqueeze(cls_codes_for_completion[index], 0),
...
# def gen_mesh(completions,
#              cls_codes_for_completion,
#              object_input_features,
#              index):
#     net_idx = torch.argmax(cls_codes_for_completion[index])
#     mesh = completions[net_idx].generator.generate_mesh(
#         torch.unsqueeze(object_input_features[index], 0),
#         torch.unsqueeze(cls_codes_for_completion[index], 0)
#     )
#     return mesh