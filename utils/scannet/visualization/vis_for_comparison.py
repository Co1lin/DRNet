...
class Vis_base(Vis_Scannet):
    ...
    def set_render_window(self, centroid, only_points, mode, min_max_dist, offline):
    ...
    def visualize(self, centroid=np.array([0, -2.5, 2.5]), save_path = None, only_points=False, mode='pred', min_max_dist=None, offline=True):
        '''
        Visualize a 3D scene.
        '''
        render_window, min_max_dist = self.set_render_window(centroid, only_points, mode, min_max_dist, offline=offline)
...
if __name__ == '__main__':
    ...
    p = Pool(processes=64)
    p.map(partial(main, vis_root, save_path), scenes)
